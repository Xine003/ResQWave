...
export function updateBoundaryFill(
  map: Map,
  coords: number[][] | null
) {
...
export function updateBoundaryVertices(
  map: Map,
  coords: number[][] | null
) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i === n - 1 && coords[0] && coords[n - 1] && coords[0][0] === coords[n - 1][0] && coords[0][1] === coords[n - 1][1]) break
    const [lng, lat] = coords[i]
    pts.push({ type: "Feature", properties: { idx: i }, geometry: { type: "Point", coordinates: [lng, lat] } })
  }

  const fc: GeoJSON.FeatureCollection<GeoJSON.Point> = { type: "FeatureCollection", features: pts }
