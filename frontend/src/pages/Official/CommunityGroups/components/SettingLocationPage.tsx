...
// Use provided Mapbox token explicitly to avoid env issues during dev
...
export default function SettingLocationPage() {
  ...
  // no info dialog state needed anymore
  ...
  useEffect(() => {
    ...
    if (phase === "terminal") {
      ...
      const draw = new MapboxDraw({ displayControlsDefault: false, controls: { line_string: true, trash: true }, defaultMode: "draw_line_string" })
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#171717] text-white">
      {/* Header */}
      <div className="absolute top-0 left-0 right-0 z-20 flex items-center gap-2 px-4 py-3 border-b border-[#2a2a2a] bg-black backdrop-blur-sm">
        <div className="font-semibold text-base">{title}</div>
        
        <div className="ml-auto flex gap-2">
          {phase === "terminal" && !terminalSaved && (
        <>
          <Button onClick={() => { if (selectedPoint) { setTerminalSaved(true); alertsRef.current?.showLocationSaved() } }} disabled={!selectedPoint} className="bg-[#4285f4] hover:bg-[#3367d6] text-white disabled:opacity-60">Save</Button>
              <Button variant="outline" onClick={handleExit} className="bg-transparent border-[#2a2a2a] text-white hover:bg-[#262626]">Exit</Button>
            </>
          )}
          {phase === "terminal" && terminalSaved && (
            <>
              <Button onClick={handleProceed} className="bg-[#4285f4] hover:bg-[#3367d6] text-white">Proceed</Button>
              <Button variant="outline" onClick={handleExit} className="bg-transparent border-[#2a2a2a] text-white hover:bg-[#262626]">Exit</Button>
            </>
          )}
          {phase === "boundary" && !boundarySaved && (
            <>
              <Button onClick={handleSaveBoundary} disabled={!hasLine} className="bg-[#4285f4] hover:bg-[#3367d6] text-white disabled:opacity-60">Save</Button>
              <Button variant="outline" onClick={handleExit} className="bg-transparent border-[#2a2a2a] text-white hover:bg-[#262626]">Exit</Button>
            </>
          )}
          {phase === "boundary" && boundarySaved && (
            <>
              <Button onClick={() => {
                ...
                navigate(-1)
              }} className="bg-[#4285f4] hover:bg-[#3367d6] text-white">Confirm</Button>
              <Button variant="outline" onClick={() => setBoundarySaved(false)} className="bg-transparent border-[#2a2a2a] text-white hover:bg-[#262626]">Discard</Button>
            </>
          )}
        </div>
      </div>
