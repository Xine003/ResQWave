...
export function useWaitlistWebSocket() {
    ...
    const handleWaitlistFormRemoved = useCallback((data: { alertId: string; rescueFormId: string; action: string }) => {
        ...
        if (data.action === 'dispatched') {
            ...
            console.log('[WaitlistWebSocket] Removing waitlisted form for alert:', data.alertId);
    ...
    useEffect(() => {
        if (!socket || !isConnected) {
            console.log('[WaitlistWebSocket] Socket not ready, skipping waitlist listeners');
            return;
        }
