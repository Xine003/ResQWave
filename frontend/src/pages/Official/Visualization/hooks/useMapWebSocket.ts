...
interface UseMapWebSocketProps {
    ...
    onWaitlistFormRemoved?: (data: { alertId: string; rescueFormId: string; action: string }) => void;
}

/**
 * Hook to listen for real-time map alerts via WebSocket
 */
export function useMapWebSocket({ onNewAlert, onAlertStatusUpdate, onWaitlistFormRemoved }: UseMapWebSocketProps) {
    ...
    const handleAlertStatusUpdate = useCallback((data: MapAlertResponse) => {
        ...
        try {
            ...
            if (!signal) {
                console.warn('[WebSocket] Could not transform alert status update:', data);
                return;
            }
            ...
            console.error('[WebSocket] Error processing alert status update:', error);
        }
    }, [onAlertStatusUpdate]);
    ...
    }, [socket, isConnected, handleMapReport, handleAlertStatusUpdate, handleWaitlistFormRemoved]);
