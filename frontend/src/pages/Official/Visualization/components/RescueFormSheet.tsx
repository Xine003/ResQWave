...
export default function RescueFormSheet({ isOpen, onClose, focalPerson = "Gwyneth Uy", alertId, onWaitlist, onDispatch }: RescueFormSheetProps) {
    ...
    const waterLevelOptions = [
        "Above Head", "Chest Deep", "Ankle Deep", "Waist Deep", "Knee Deep"
    ];

    const urgencyOptions = [
        "Immediate evacuation needed", "Urgent - within 1 hour", "Soon - within 4 hours",
        "Safe for now", "Monitoring Situation"
    ];

    const hazardOptions = [
        "Live electrical wires", "Strong water current", "Dangerous debris",
        "Structural damage", "Chemical spills"
    ];

    const accessibilityOptions = [
        "Vehicle accessible", "Boat accessible", "Walking accessible", "All access blocked"
    ];

    const resourceOptions = [
        "Rescue Boat", "Medical assistance", "Additional personnel", "Standard rescue team"
    ];
    ...
    return (
        <>
            <div
                className={`fixed top-0 right-0 h-full w-[400px] bg-[#171717] border-l border-[#2a2a2a] transform transition-transform duration-300 ease-in-out ${(isOpen && !isRescuePreviewOpen) ? 'translate-x-0' : 'translate-x-full'
                    }`}
                style={{ zIndex: 60 }}
            >
                {/* Header */}
                ...
                <div className="flex-1 overflow-y-auto px-6 pb-24 h-[calc(100vh-160px)]">
                    ...
                    <div className="mb-6 pt-6">
                        <label className="block text-gray-300 text-sm mb-2">Focal Person</label>
                        <input
                            type="text"
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                            {waterLevelOptions.map((option) => (
                                <button
                                    ...
                                    className={`px-3 py-2 rounded text-sm transition-colors ${waterLevel === option
                                            ? 'bg-white text-black'
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                            {urgencyOptions.map((option) => (
                                <button
                                    ...
                                    className={`px-3 py-2 rounded text-sm transition-colors ${urgencyLevel === option
                                            ? 'bg-white text-black'
                                            : 'bg-[#2a2a2a] text-gray-300 hover:bg-[#333333]'
                                        }`}
                                >
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                            {hazardOptions.map((option) => (
                                <button
                                    ...
                                    className={`px-3 py-2 rounded text-sm transition-colors ${hazards.includes(option)
                                            ? 'bg-white text-black'
                                            : 'bg-[#2a2a2a] text-gray-300 hover:bg-[#333333]'
                                        }`}
                                >
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                            {accessibilityOptions.map((option) => (
                                <button
                                    ...
                                    className={`px-3 py-2 rounded text-sm transition-colors ${accessibility === option
                                            ? 'bg-white text-black'
                                            : 'bg-[#2a2a2a] text-gray-300 hover:bg-[#333333]'
                                        }`}
                                >
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                            {resourceOptions.map((option) => (
                                <button
                                    ...
                                    className={`px-3 py-2 rounded text-sm transition-colors ${resources.includes(option)
                                            ? 'bg-white text-black'
                                            : 'bg-[#2a2a2a] text-gray-300 hover:bg-[#333333]'
                                        }`}
                                >
