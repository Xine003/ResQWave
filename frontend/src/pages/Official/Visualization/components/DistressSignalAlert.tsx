...
export default forwardRef<DistressSignalAlertHandle, DistressSignalAlertProps>(
	function DistressSignalAlert({ onGoToLiveReport }, ref) {
		...
		const getAlertMessage = () => {
			...
			const alertTypeText = alertData.alertType === 'CRITICAL' ? 'critical' : 'user-initiated'
			const locationText = alertData.focalPerson ? `from ${alertData.focalPerson}` : 
				alertData.deviceId ? `from ${alertData.deviceId}` : 'in your area'
		...
		return (
			<>
				...
				<div className={`fixed top-[73px] left-1/2 transform -translate-x-1/2 z-50 transition-all duration-600 ease-out ${showAlert ? "translate-y-0 opacity-100" : "-translate-y-8 opacity-0 pointer-events-none"}`}>
