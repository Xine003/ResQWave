...
import { Monitor, MonitorCheck, Plus, X } from "lucide-react"
import { forwardRef, useEffect, useImperativeHandle, useRef, useState } from "react"

export type TerminalAlertsHandle = {
	showCreateSuccess: (terminalName: string) => void
	showUpdateSuccess: (terminalName: string) => void
	showDeleteSuccess: (terminalName: string) => void
	showArchiveSuccess: (terminalName: string) => void
	showRestoreSuccess: (terminalName: string) => void
	showError: (message: string) => void
	showDeleteConfirmation: (terminalId: string, terminalName: string, onConfirm: () => void) => void
	hideAll: () => void
}
...
export default forwardRef<TerminalAlertsHandle, {}>(function TerminalAlerts(_props, ref) {
	...
	useImperativeHandle(ref, () => ({
		showCreateSuccess: (terminalName: string) => {
			hideAllAlerts()
			setCreateMessage(`Terminal "${terminalName}" created successfully!`)
			setShowCreate(true)
		...
		showUpdateSuccess: (terminalName: string) => {
			hideAllAlerts()
			setUpdateMessage(`Terminal "${terminalName}" updated successfully!`)
			setShowUpdate(true)
		...
		showDeleteSuccess: (terminalName: string) => {
			hideAllAlerts()
			setDeleteMessage(`Terminal "${terminalName}" deleted permanently!`)
			setShowDelete(true)
		...
		showArchiveSuccess: (terminalName: string) => {
			hideAllAlerts()
			setArchiveMessage(`Terminal "${terminalName}" archived successfully!`)
			setShowArchive(true)
		...
		showRestoreSuccess: (terminalName: string) => {
			hideAllAlerts()
			setRestoreMessage(`Terminal "${terminalName}" restored successfully!`)
			setShowRestore(true)
		...
		showDeleteConfirmation: (terminalId: string, _terminalName: string, onConfirm: () => void) => {
			setDeleteConfirmId(terminalId)
	...
	return (
		<>
				...
				<Alert className="min-w-[280px] max-w-[520px] bg-[#171717] border border-[#2a2a2a] text-white rounded-[5px] !items-center !grid-cols-[auto_1fr] !gap-x-3">
					<div className="flex h-12 w-12 items-center justify-center rounded-[5px] bg-green-600/25">
						<div className="relative">
							...
							<Plus className="absolute -top-1 -right-1 size-3 text-[#22c55e]" />
						</div>
					</div>
			...
			<div className={`fixed left-[85px] bottom-[30px] z-50 transition-all duration-300 ease-out ${showUpdate ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0 pointer-events-none"}`}>
				<Alert className="min-w-[280px] max-w-[520px] bg-[#171717] border border-[#2a2a2a] text-white rounded-[5px] !items-center !grid-cols-[auto_1fr] !gap-x-3">
					<div className="flex h-12 w-12 items-center justify-center rounded-[5px] bg-blue-600/25">
						<MonitorCheck className="size-5 text-[#3B82F6]" />
					</div>
					<AlertDescription className="text-[13px] leading-tight">
						{updateMessage}
					</AlertDescription>
			...
			{/* Delete Success Alert - Bottom Left */}
			<div className={`fixed left-[85px] bottom-[30px] z-50 transition-all duration-300 ease-out ${showDelete ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0 pointer-events-none"}`}>
				<Alert className="min-w-[280px] max-w-[520px] bg-[#171717] border border-[#2a2a2a] text-white rounded-[5px] !items-center !grid-cols-[auto_1fr] !gap-x-3">
					<div className="flex h-12 w-12 items-center justify-center rounded-[5px] bg-red-600/25">
						<div className="relative">
							<Monitor className="size-5 text-[#ef4444]" />
			...
			{/* Archive Success Alert - Bottom Left */}
			<div className={`fixed left-[85px] bottom-[30px] z-50 transition-all duration-300 ease-out ${showArchive ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0 pointer-events-none"}`}>
				<Alert className="min-w-[280px] max-w-[520px] bg-[#171717] border border-[#2a2a2a] text-white rounded-[5px] !items-center !grid-cols-[auto_1fr] !gap-x-3">
					<div className="flex h-12 w-12 items-center justify-center rounded-[5px] bg-orange-600/25">
						<div className="relative">
							<Monitor className="size-5 text-[#f97316]" />
			...
			{/* Restore Success Alert - Bottom Left */}
			<div className={`fixed left-[85px] bottom-[30px] z-50 transition-all duration-300 ease-out ${showRestore ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0 pointer-events-none"}`}>
				<Alert className="min-w-[280px] max-w-[520px] bg-[#171717] border border-[#2a2a2a] text-white rounded-[5px] !items-center !grid-cols-[auto_1fr] !gap-x-3">
					<div className="flex h-12 w-12 items-center justify-center rounded-[5px] bg-green-600/25">
						<MonitorCheck className="size-5 text-[#22c55e]" />
					</div>
					<AlertDescription className="text-[13px] leading-tight">
						{restoreMessage}
					</AlertDescription>
					...
					<div className="bg-[#171717] border border-[#2a2a2a] rounded-[5px] p-6 max-w-md w-full mx-4 shadow-xl">
						<div className="flex items-start gap-4">
							<div className="flex h-12 w-12 items-center justify-center rounded-[5px] bg-red-600/25 flex-shrink-0">
								<div className="relative">
									<Monitor className="size-6 text-[#ef4444]" />
									<X className="absolute -top-1 -right-1 size-3 text-[#ef4444]" />
								</div>
							</div>
							<div className="flex-1">
								...
								<p className="text-[14px] text-[#a1a1a1] mb-1">
									Are you sure you want to permanently delete terminal <span className="text-white font-semibold">{deleteConfirmId}</span>?
								</p>
