...
import { InputOTP, InputOTPGroup, InputOTPSeparator, InputOTPSlot } from '@/components/ui/input-otp-focal';
...
export function VerifyCode({ userID, maskedEmail, onVerified, onBack, onResend, onError }: VerifyCodeProps) {
  ...
  function getStoredExpiry() {
    const stored = sessionStorage.getItem('passwordResetExpiry');
    if (stored) {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      await verifyResetCode(userID, code);
      // Store verified code in sessionStorage
      sessionStorage.setItem('passwordResetCode', code);
      onVerified(code);
    } catch (err: unknown) {
      const errorObj = err as { message?: string; status?: number; data?: { message?: string; failedAttempts?: number; locked?: boolean } };
      
      if (errorObj.status === 429) {
        // Account locked
        onError(errorObj.message || 'Too many attempts. Account temporarily locked. Please try again later.');
        setCode(''); // Clear code on lock
      } else if (errorObj.status === 400) {
        // Invalid code or expired - parse the error message
        const errorMessage = errorObj.message || errorObj.data?.message || 'Invalid code. Please try again.';
  ...
  const handleResend = async () => {
    ...
    try {
      // Get original email/number from sessionStorage (not the masked one)
      const originalEmailOrNumber = sessionStorage.getItem('passwordResetEmailOrNumber');
      if (!originalEmailOrNumber) {
        onError('Session expired. Please start over.');
        setIsResending(false);
        return;
      }
      
      await requestPasswordReset(originalEmailOrNumber);
      
      // Reset error and code
      setError('');
      setCode('');
      ...
      onError(error.message || 'Failed to resend code. Please try again.');
    } finally {
      setIsResending(false);
    }
  };
    ...
    <div>
      ...
      {showResendAlert && (
        <div style={{ position: 'fixed', left: 30, bottom: 30, zIndex: 100000, minWidth: 220, maxWidth: 520, transition: 'opacity 220ms linear', opacity: showResendAlert ? 1 : 0 }}>
          <div className="bg-[#171717] border border-[#2a2a2a] text-white rounded-[5px] p-4 flex items-center gap-3">
      ...
      >
        ...
        >
          ...
          {error && (
            <p className="text-red-500 text-sm mb-3 text-center" style={window.innerWidth <= 480 ? { fontSize: '0.75rem' } : {}}>
              {error}
            </p>
          )}

          <div className="text-center mb-3">
            <span className="text-[#BABABA] text-sm">Didn't receive any code? </span>
            <button
              type="button"
              onClick={handleResend}
              disabled={isResending || expiresIn > 0}
              className={`text-[#3B82F6] text-sm bg-transparent border-none disabled:opacity-60 ${
                isResending || expiresIn > 0 ? 'cursor-default' : 'hover:text-[#2C64C5] cursor-pointer'
              }`}
            >
