...
interface FindAccountProps {
  onNext: (userID: number, maskedEmail: string, expiryTimestamp: number) => void;
  onBack: () => void;
...
export function FindAccount({ onNext, onBack, onError }: FindAccountProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await requestPasswordReset(emailOrNumber.trim());
      
      // Calculate expiry timestamp
      const expiryTimestamp = Date.now() + response.expiresInMinutes * 60 * 1000;
      
      // Store in sessionStorage for persistence
      sessionStorage.setItem('passwordResetExpiry', expiryTimestamp.toString());
      sessionStorage.setItem('passwordResetUserID', response.userID.toString());
      sessionStorage.setItem('passwordResetMaskedEmail', response.maskedEmail);
      sessionStorage.setItem('passwordResetEmailOrNumber', emailOrNumber.trim()); // Store original for resend
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            Enter your email address or your phone number to<br />
        ...
        >
          ...
          />
          {error && (
            <p className="text-red-500 text-sm mb-3" style={window.innerWidth <= 480 ? { fontSize: '0.75rem' } : {}}>
              {error}
            </p>
          )}
