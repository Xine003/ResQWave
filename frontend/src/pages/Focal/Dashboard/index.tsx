...
import {
  createDraw,
  ensureSquareGreenImage,
  changeToDrawPolygon,
  makeUpdateCanSave,
} from "./utils/drawMapBoundary";
...
mapboxgl.accessToken =
  "pk.eyJ1Ijoicm9kZWxsbCIsImEiOiJjbWU0OXNvb2gwYnM0MnpvbXNueXo2dzhxIn0.Ep43_IxVhaPhEqWBaAuuyA";
...
export default function Dashboard() {
  ...
  const {
    ...
    setCanSave,
    getDistressCoord,
  } = signals as unknown as DashboardSignals;
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
        ...
        floodFiles.forEach((file) => {
          ...
          if (!map.getLayer(polygonLayerId)) {
            map.addLayer(
              ...
              "waterway-label"
            ); // 👈 ensures labels stay above
          }
        });
      } catch (e) {
        console.warn(
          "[Dashboard] could not add flood polygons sources/layers",
          e
        );
      }
      ...
      const distressLayers = [
        "distress-core",
        "distress-core-stroke",
        "distress-ring-1",
        "distress-ring-2",
        "distress-ring-3",
      ];
        ...
        layerIds.forEach((layerId) => {
          map.on(
            ...
            (e: mapboxgl.MapMouseEvent & { features?: GeoJSON.Feature[] }) => {
              ...
              if (map.getLayer("signal-radius"))
                map.removeLayer("signal-radius");
              if (map.getSource("signal-radius"))
                map.removeSource("signal-radius");
              ...
              if (status === "offline") {
                ...
                if (deviceId === OwnCommunitySignal.properties.deviceId) {
                  ...
                  const found = otherSignals.find(
                    (s) => s.properties.deviceId === deviceId
                  );
              ...
              if (typeof extendedMap.createGeoJSONCircle === "function") {
                ...
                map.addSource("signal-radius", {
                  type: "geojson",
                  data: extendedMap.createGeoJSONCircle(
                    coord,
                ...
                function animatePulse(startTime: number) {
                  ...
                  const currentRadius =
                    pulseConfig.minRadius +
                    (pulseConfig.maxRadius - pulseConfig.minRadius) * eased;
                  ...
                  if (source && extendedMap.createGeoJSONCircle) {
                    source.setData(
                      extendedMap.createGeoJSONCircle(coord, currentRadius)
                    );
                  }
                  ...
                  if (t < 1) {
                    extendedMap._pulseFrame = requestAnimationFrame(() =>
                    ...
                    if (source && extendedMap.createGeoJSONCircle) {
                      source.setData(
                        extendedMap.createGeoJSONCircle(
                          coord,
                          pulseConfig.maxRadius
                        )
                      );
                    }
                    map.setPaintProperty(
  ...
  useEffect(() => {
    ...
    if (
      Array.isArray(distressCoord) &&
      (distressCoord[0] !== 0 || distressCoord[1] !== 0)
    ) {
  ...
  useEffect(() => {
    ...
    if (editBoundaryOpen) {
      ...
      if (map.getLayer("signal-boundary-line"))
        map.removeLayer("signal-boundary-line");
  ...
  const handleSave = () => {
    ...
    if (
      polygons[0] &&
      polygons[0].geometry &&
      polygons[0].geometry.type === "Polygon" &&
      Array.isArray(polygons[0].geometry.coordinates)
    ) {
    ...
    try {
      ...
      if (map && newBoundary) {
        ...
        if (map.getLayer("signal-boundary-line"))
          map.removeLayer("signal-boundary-line");
        if (map.getSource("signal-boundary"))
          map.removeSource("signal-boundary");

        // determine the actual signal point coordinate (use the edited signal's point)
        let signalCoord: [number, number] | null = null;
        const targetDeviceId =
          popover?.deviceId ?? OwnCommunitySignal.properties.deviceId;
        ...
        if (targetDeviceId === OwnCommunitySignal.properties.deviceId) {
          ...
          const found = otherSignals.find(
            (s: Signal) => s.properties?.deviceId === targetDeviceId
          );
          if (found) {
            sourceSignalFeature = found;
            if (found.coordinates)
              signalCoord = found.coordinates as [number, number];
          }
        }
        ...
        setPopover({
          ...
          title:
            propsFromSignal.name ||
            popover?.title ||
            (propsFromSignal.status === "offline"
              ? "Offline Signal"
              : "Community"),
          ...
          altFocalPerson:
            propsFromSignal.altFocalPerson || popover?.altFocalPerson,
        });
      }
    } catch (e) {
      console.warn(
        "[Dashboard] could not programmatically show saved boundary popover",
        e
      );
    }
  ...
  const [activityLogCenter, setActivityLogCenter] = useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  const [accountSettingsCenter, setAccountSettingsCenter] = useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  const handleModalSwitchWithDirtyCheck = useCallback(
    (openModalFn: () => void) => {
      if (
        accountSettingsOpen &&
        (accountSettingsIsDirtyRef.current?.() ?? false)
      ) {
          ...
          if (editAboutOpen) {
            editAboutRef.current?.openDiscardConfirm?.(() => {
              ...
              try {
                const rect = mapContainer.current?.getBoundingClientRect();
                if (rect)
                  setAccountSettingsCenter({
                    x: rect.left + rect.width / 2,
                    y: rect.top + rect.height / 2,
                  });
                else setAccountSettingsCenter(null);
              } catch {
                setAccountSettingsCenter(null);
              }
          ...
          if (editAboutOpen) {
            editAboutRef.current?.openDiscardConfirm?.(() => {
              ...
              try {
                const rect = mapContainer.current?.getBoundingClientRect();
                if (rect)
                  setActivityLogCenter({
                    x: rect.left + rect.width / 2,
                    y: rect.top + rect.height / 2,
                  });
                else setActivityLogCenter(null);
              } catch {
                setActivityLogCenter(null);
              }
        ...
        <EditAboutCommunity
          ...
          onSave={(updatedData: any) => {
            ...
            if (
              ...
              const altName = [
                updatedData.altFirstName,
                updatedData.altLastName,
              ]
                .filter(Boolean)
                .join(" ");
              setPopover({
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You have unsaved changes in your account settings. Are you sure
              ...
              onClick={() => {
                ...
                try {
                  if (pendingModalContinueRef.current) {
                    ...
                    setAccountSettingsOpen(false);
                    try {
                      pendingAccountContinueRef.current();
                    } catch {
